<template>
  <div>
    <Navbar class="nav-bar">
      <div slot="left" @click="goLogin"><span class="iconfont icon-tubiaozhizuo--"></span></div>
      <div slot="center" class="reg">注册</div>
    </Navbar>
    <div class="input">
      <input type="text" placeholder="请输入账号" @blur="blurAccount" v-model="msg.user_account">
      <p :class="{isOk:isAccount}">{{Account}}</p>
    </div>
    <div class="input">
      <input type="password" placeholder="请输入密码" v-model="msg.user_password" @blur="blurPassword">
      <p :class="{isOk:isPassword}">{{Password}}</p>
    </div>
    <div class="finalGister" @click="goRegister">注册</div>
    <loading :isShow='isLoading' :radius='30' :lineWidth='10'></loading>
  </div>
</template>
