<template>
  <div class="good">
    ...
    <div class="select" @click="goBuy">
      <div>请选择：尺码</div>
      <div><span class="iconfont icon-qianjin" style="font-size:12px"></span></div>
    </div>
    <!-- reconmend 评价部分 -->
    <div class='reconmend'>
...
<script>
import HeadApp from "../../components/header/head";
// import "./good.css";
import Swiper from "swiper";
import "swiper/swiper-bundle.css";
import Swiper2, { Navigation, Pagination } from "swiper"; // 这行代码很关键
Swiper2.use([Navigation, Pagination]); // 这行代码很关键
import { goodsdetail } from "../../api/goodsapi";
import {addcart} from "../../api/cartapi"
import {getuserinfo,addcollect,delcollect} from "../../api/adminapi"
...
export default {
  ...
  methods: {
    ...
    changeType(item){  //点击改变商品的颜色类型
      this.isShop_type = item
    },
    changeSize(item){ //点击改变商品选择的尺寸
    ...
    async addCart(){
      ...
      // console.log(obj)
      //前端验证  递交购物车
      const result = await addcart(this.$qs.stringify(obj))
      // console.log(result)
    ...
    async IsCollect(){
      ...
      let id=this.$route.params.id  //商品的id
    ...
    async collect(){
      //
       //没有收藏 递交接口
           if(!this.isCollect){
                const res=await addcollect({uid: this.$route.params.id})
                // console.log(res)
                this.isCollect = true
           }else{  //删除收藏
                const res=await delcollect({uid: this.$route.params.id})
                // console.log(res)
                this.isCollect = false
           }
    },
