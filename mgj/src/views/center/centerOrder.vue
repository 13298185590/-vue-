  ...
  <div class="profileOrder">
    <NavBar class="narbar">
      <div slot='left' @click="goBack">
        <span class="iconfont icon-tubiaozhizuo--" style="fontsize:25px"></span>
      </div>
      <div slot="center">订单列表</div>
    </NavBar>
    <div class="orderDes">
      <div v-for="(item,index) in orderDes" :key="index" @click="orderIndex=index">
        <span :class="{border:index==orderIndex}">{{item}}</span>
      </div>
    </div>
    <!-- good order -->
    <div class="goods">
      <div v-if="!selectOrder.length" class="noneGoods"> 
        <img src="https://s10.mogucdn.com/new1/v1/bmisc/b22401c122fbedee3b1f5d708127e9f0/194500731154.png"
             alt="">
        <div>还没有相关订单哦</div>
      </div>
      <div v-if="selectOrder.length">
        <div v-for="(item,index) in selectOrder" :key="index" class="goodsList">
