<template>
    <div class="cart">
      ...
      <div class='cart-list-head'>
        ...
        <div class="shopname">{{item}}</div>
      </div>
      <div class='cart-list-item'
           v-for='(item1,index1) in cartList[item]'
           :key='index1' >
        <span class='delete' @click='deleteCartone(item,index)'>x</span>
    ...
    <div class='bottom-nar'>
      ...
      <div style='width:200px'>全选({{isAllNum}})</div>
      <div>￥{{isAllPrice}} </div>
      <div @click='goOrder' class="jiesuan">结算</div>
    </div>
...
import {getuserinfo,deletecart} from '../../api/adminapi'
export default {
    components:{
      navBar
    },
    ...
    methods:{
      ...
      //删除商品
      async deleteCartone(obj,index){
        console.log(obj)
        console.log(index)
        const result=await deletecart(this.$qs.stringify({obj:obj,index:index}))
        console.log(result)
        //this.getCart()
